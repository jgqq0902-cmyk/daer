...
- 正式牌局规则推进必须由 K 工作区内的 `packages/core` 通过本地 Node/TS Bridge 统一处理；Godot UI 不得自行修改牌局状态或重复实现规则判断。`scripts/game_service.gd` 仅作为 `--test` 离线夹具，不得加入正式自动加载或运行时牌局路径。E 工作区不属于当前 Godot 项目的运行时或编辑依赖。
