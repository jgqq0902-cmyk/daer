...
过张采用独立的 GUO 规则 ID，不能只用“过张不可吃”概括：

- **GUO-01**：主动打出某牌，自己记录该 `rank + size`，之后不可吃或胡同牌面。
- **GUO-02**：仅在确有合法吃牌方案且玩家主动选择“过”时记录过张；系统无动作推进不记录。
- **GUO-03**：过张后禁止再次吃同一 `rank + size`。
- **GUO-04**：过张后禁止以同一 `rank + size` 胡牌，core 与 Bridge 均需防御。
- **GUO-05**：无合法吃方案时不得通过自动过、无响应或出牌者排除伪造过张。
- **GUO-06**：大、小牌按 `rank + size` 独立，过小六不限制大六。
- **GUO-07**：绕过 UI 直接提交非法 `chi/hu` 必须拒绝且状态不变；重复记录同牌面应幂等。

正式生命周期固定为整局生效（`guoZhangClearPolicy = NEVER`）；历史文档中“每回合清除”描述废止。
